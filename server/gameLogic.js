...
class GameRoom {
  ...
  moveToken(socketId, tokenIndex) {
    ...
    // ── WIN CHECK: exact finish only ─────────────────────────
    // token must land EXACTLY on position 57
    if (token.position === 57) {
      token.finished = true;
      player.finishedTokens++;
    } else if (token.position > 57) {
      // Overshoot — should not happen due to _movable check, but guard
      token.position -= this.lastDiceValue;
      this.diceRolledThisTurn = false;
      this.movableTokens = [];
    ...
    // Kill check on outer track (pos 0–50)
    ...
    else if (this.lastDiceValue===6 && extraTurn) {} // already true
    ...
    // Sync finishedTokens to be accurate

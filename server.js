...
io.on('connection', (socket) => {
  ...
  socket.on('randomMatch', ({ playerName, preferredColor }) => {
    ...
    // AUTO-START for random rooms: countdown when 2+ players joined
    if (room.playerCount() >= 2 && !room.gameStarted) {
      ...
      let secs = 5; // 5-second countdown (so more players can join)
      io.to(rid).emit('autoStartCountdown', { seconds: secs, players: room.getPlayersInfo() });
      ...
      room._countdownTimer = setInterval(() => {
        ...
        if (secs > 0) {
          ...
          clearInterval(room._countdownTimer);
          room._countdownTimer = null;
          // Only start if still has 2+ players and not already started

...
socket.on('playerJoined', ({ players, gameState, yourPlayerId, colorAssigned, roomId, roomType }) => {
  ...
  // Custom room → show Start button when 2+ players
  // Random room  → auto-starts, no start button needed
  if (roomType === 'custom') {
    $('start-btn').classList.toggle('hidden', players.length < 2);
  } else {
    $('start-btn').classList.add('hidden'); // random = auto-start
  }
  $('waiting-text').textContent = players.length < 2
    ? `${players.length}/4 joined — min 2 players chahiye`
    : roomType === 'random'
      ? `${players.length} players ready — game jald shuru hoga! ⏳`
      : `${players.length} players ready! Start karo 🚀`;
});

socket.on('readyToStart', ({ players, roomType }) => {
  const rt = roomType || currentRoomType;
  _renderWaiting(players, rt, currentRoomId);
  if (rt !== 'random') {
...
/* ── autoStartCountdown (random rooms only) ─────────────────── */
socket.on('autoStartCountdown', ({ seconds, players }) => {
  // Update player list
  _renderWaiting(players, 'random', currentRoomId);

  const txt = $('waiting-text');
  if (!txt) return;

  if (seconds > 0) {
    txt.innerHTML =
      `<span style="color:#a78bfa;font-size:15px;font-weight:900">`+
      `⏳ Game shuru hoga ${seconds} second mein…</span><br/>`+
      `<span style="font-size:12px;color:#5555a0">(${players.length} players joined)</span>`;
  } else {
    txt.innerHTML = `<span style="color:#4ade80;font-weight:900">🚀 Starting…</span>`;
  }
});

...
rollBtn.addEventListener('click', () => {
  if (inOffline) return;       // offline.js handles this
...
function _refreshRollBtn(gs, overrideTurn) {
  if (!gs) { rollBtn.disabled = true; return; }
  ...
  rollBtn.disabled = !(isMyTurn && !alreadyRolled);
}
...
function setHint(msg) {
  if (hintLabel) hintLabel.textContent = msg;
}

...
function initBoard() {
  ...
  let S;
  if (_isDesktop) {
    // Desktop: fill center column minus sidebars
    const sideW  = 180 * 2 + 16 * 2;
    const maxW   = window.innerWidth  - sideW - 24;
    const maxH   = window.innerHeight - 24;
    S = Math.min(maxW, maxH);
  } else {
    // Mobile: fill the board-main area exactly
    const boardMain = document.querySelector('.board-main');
    const rect      = boardMain ? boardMain.getBoundingClientRect() : null;
    const vw = rect ? rect.width  : window.innerWidth;
    const vh = rect ? rect.height : (window.innerHeight - 56 - 52); // minus top/bottom bars
    S = Math.min(vw, vh) - 8;
  }
  ...
  if (topBar) topBar.style.display = _isDesktop ? 'none' : '';
  if (botBar) botBar.style.display = _isDesktop ? 'none' : '';
